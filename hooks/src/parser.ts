...
import { Token, tokenizeEnhanced } from "./tokenizer"
...
function parseNoteMods(content: string): ParsedNoteMods {
  ...
  return { selfArticulations: self, transitionArticulation, hasBend, isHammerOrPull, isSlide, isTrill }
}
...
class TokenCursor {
  ...
  consume(): Token {
    if (this.pos >= this.tokens.length) throw new Error("Unexpected end of token stream")
...
function collectNoteEventTokens(cursor: TokenCursor): { noteMod?: string; durMod?: string; noteValue: number } {
...
export function parse(source: string): ParsedPiece {
  const tokens = tokenizeEnhanced(source)
  ...
  while (cursor.hasMore()) {
      ...
      switch (directive) {
        ...
        case "ts": {
          ...
          if (!headerParsed) {
            ...
            state.currentTimeSig = ts
            timeSignatureChanges.push({ atNoteId: state.noteIdCounter, numerator: ts[0], denominator: ts[1] })
          }
          break
        }
    ...
    if (token.type === "REST") {
      ...
      const durMods = durMod ? parseDurMods(durMod) : { dotted: false, tuplet: undefined }
      ...
      state.currentBeatInMeasure += (4 / noteValue) * (durMods.dotted ? 1.5 : 1) * (durMods.tuplet ? durMods.tuplet.normal / durMods.tuplet.actual : 1)
      continue
    }
    ...
    if (token.type === "NOTE") {
      ...
      const { noteMod, durMod, noteValue } = collectNoteEventTokens(cursor)
      const noteMods = noteMod ? parseNoteMods(noteMod) : {
        selfArticulations: [], transitionArticulation: undefined,
        hasBend: false, isHammerOrPull: false, isSlide: false, isTrill: false
      }
      const durMods = durMod ? parseDurMods(durMod) : { dotted: false, tuplet: undefined }

      const durationSeconds = noteValueToSeconds(
        noteValue, state.currentTempo, durMods.dotted, durMods.tuplet
      )

      // Filter fret=0 (open string, no left-hand involvement)
      const placements: NotePlacement[] = fret === 0
        ? []
        : [{ string: stringNum, fret }]
    ...
    if (token.type === "CHORD_OPEN") {
      ...
      while (cursor.hasMore() && cursor.peek()?.type !== "CHORD_CLOSE") {
        if (cursor.peek()?.type === "NOTE") {
          ...
          if (perNoteMods?.transitionArticulation) {
            perNoteTransitions.set(stringNum, perNoteMods.transitionArticulation)
          }
        } else {
      ...
      const { noteMod, durMod, noteValue } = collectNoteEventTokens(cursor)
      const chordNoteMods = noteMod ? parseNoteMods(noteMod) : {
        selfArticulations: [], transitionArticulation: undefined,
        hasBend: false, isHammerOrPull: false, isSlide: false, isTrill: false
      }
      const durMods = durMod ? parseDurMods(durMod) : { dotted: false, tuplet: undefined }
      ...
      const resolvedTransition = perNoteTransitions.size > 0
  ...
  for (let i = 0; i < notes.length; i++) {
    ...
    for (let j = i + 1; j < notes.length; j++) {
      ...
      const matchingPlacement = candidate.placements.find(p => p.string === transString)
      if (matchingPlacement) {
        ...
        if (note.transitionArticulation.type === "hammer-on") {
          const sourcePlacement = note.placements.find(p => p.string === transString)
